...
//        dd($detail);
    ...
    {
        return $this->excel(1, '送货单');
    }


    public function havePushExcel()
    {
        return $this->excel(2, '已推送配货单');
    }


    public function excel($status, $singleNameNote)
    {
        if ($this->request->isMethod('post')){
            $id  = Request()->input('id');
            $id = rtrim($id,',');
...
//                DB::table('dic_order')->whereIn('id',$id_arr)->update(['status'=>2]);
                return $this->export($data, $singleNameNote);
            }else{
                return ['code'=>1,'url'=>''];
            }

        }else{
            $data = DB::table('dic_order')->where('status',$status)->orderBy('id', 'DESC')->paginate(15);
            return view('admin/distribution/index',['data'=>$data]);
        }
    }


    public function export($data, $singleNameNote){
...
//返回已经存好的文件目录地址提供下载
    ...
    public function havePushManyExcel()
    {
        return $this->manyExcel(2, '已推送配货单');
    }

    public function manyExcel($status, $singleNameNote)
    {
        if ($this->request->isMethod('post')){
...
//            $code = json_decode($httpResult['data'],true)['Code'];
            ...
            if ($code==200){
//                DB::table('dic_order')->whereIn('id',$id_arr)->update(['status'=>2]);
                return $this->exportMany($data, $singleNameNote);
            }else{
                return ['code'=>1,'url'=>''];
            }
    ...
    public function exportMany($data, $singleNameNote){
...
//        dd($data);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('F5', '金额');
        $objPHPExcel->getActiveSheet()->setCellValue('G5', '科室名称');
        $objPHPExcel->getActiveSheet()->setCellValue('H5', '订单编号');
        $objPHPExcel->getActiveSheet()->setCellValue('I5', '制单人');
        $objPHPExcel->getActiveSheet()->setCellValue('J5', '总金额');
        $objPHPExcel->getActiveSheet()->setCellValue('K5', '备注');
        $objPHPExcel->getActiveSheet()->setCellValue('L5', '');
...
//            $objPHPExcel->getActiveSheet()->setCellValue('A' . $row_num, $value['Id']);
            ...
            foreach ($value['DistributionDetail'] as $v){
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('F' . $row_num, $v['Amount']);
                $objPHPExcel->getActiveSheet()->setCellValue('G' . $row_num, $v['DeptName']);
                $objPHPExcel->getActiveSheet()->setCellValue('H' . $row_num, $value['PurchaseOrderNo']);
                $objPHPExcel->getActiveSheet()->setCellValue('I' . $row_num, $value['Creator']);
                $objPHPExcel->getActiveSheet()->setCellValue('J' . $row_num, $v['Amount']);
            }
            $row_num = $row_num + abs($count - 1) ;
            $row_num++;
...
//            );
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $last, '总金额：' . $total);
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $last, '送货单位及经手人：');
        $objPHPExcel->getActiveSheet()->setCellValue('J' . $last, '收货单位及经手人：');
